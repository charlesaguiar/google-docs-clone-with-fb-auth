...
const FIREBASE_ERROR_MAP = {
	...
	'auth/too-many-requests': 'Too many failed login attemps. Account temporarilt blocked',
}

export const parseFirebaseErrorMessage = (error: AuthError): string => {
	const message = error.message
	let parsedMessage = 'An error happened. Please try again'

	console.log({ message })
	for (const k of Object.keys(FIREBASE_ERROR_MAP)) {
